...
impl LsmStorageInner {
    ...
    pub fn force_freeze_memtable(&self, _state_lock_observer: &MutexGuard<'_, ()>) -> Result<()> {
        let new_mem_table = Arc::new(MemTable::create(self.next_sst_id()));
        {
            ...
            let old_mem_table = std::mem::replace(&mut state.memtable, new_mem_table.clone());
    ...
    pub fn force_flush_next_imm_memtable(&self) -> Result<()> {
        ...
        let wait_to_flush_memtable = {
            // remenber to release the read lock
            let guard = self.state.read();
            guard.as_ref().imm_memtables.last().unwrap().clone()
        };
        let mut sst_builder = SsTableBuilder::new(self.options.block_size);
        wait_to_flush_memtable.flush(&mut sst_builder)?;

        let id = wait_to_flush_memtable.id();
        let cache = self.block_cache.clone();
        let sst_path = self.path_of_sst(id);

        // bulder
        let sst = Arc::new(sst_builder.build(id, Some(cache), sst_path)?);
        {
            let mut guard = self.state.write();
